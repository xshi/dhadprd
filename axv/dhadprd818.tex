...
\author{K.~W.~Edwards}
\affiliation{Carleton University, Ottawa, Ontario K1S 5B6, Canada}
\author{R.~A.~Briere}
\author{H.~Vogel}
...
fraction of DT events in our ST samples.
 ...
 $\Dz\Dzbar$ or $\Dp\Dm$) produced in the experiment, the observed
 yields, $\Ni$ and $\Njbar$, of reconstructed $D\to i$ and
 $\Dbar\to\jbar$ ST events will be
\begin{equation}
\label{eq:st}
...
j$, with the assumption that charge-conjugation parity ($CP$)
violation is negligible so that $\Bj = \Bjbar$. However, the
efficiencies $\effj$ and $\effjbar$ for detection of these modes may
not be the same due to the charge dependencies of cross sections for
the scattering of pions and kaons on the nuclei of the detector
material.  Furthermore, the DT yield for $D\to i$ (signal mode) and
$\Dbar\to\jbar$ (tagging mode) will be
\begin{equation}
\label{eq:dt}
...
linear scales (which emphasize signals) and logarithmic scales (which emphasize backgrounds).  This property results from the fact all error bars that are proportional to $\sqrt{N}$ are the same size on a square-root scale.  However, the error bars in these plots for small numbers of events are somewhat larger than the others because we utilize ROOFIT~\protect\cite{roofit} to produce these plots and ROOFIT error bars are 68\% confidence intervals.}
for the nine
...
to the appropriate $\NDDbar$ values that we obtain in the fit, and the branching fractions for the modes that we obtain from the particle data group \cite{pdg2010}.  For both types 
...
\begin{table*}[htb]
  \caption{ Single tag efficiencies, yields from data, and peaking
    ...
    indicated by three center dots in the ``Peaking background'' column.
...
\begin{figure*}
\includegraphics*[width=\linewidth]{3971013-004a}
...
Canada, and the U.K Science and Technology Facilities Council. X. Shi
thanks the High Energy Group National Taiwan University National
Science Council where part of this work was completed.
...
\bibitem [{cle()}]{cleocyb}%
  ...
  Collaboration, Cornell LEPP Report No. CLNS~01/1742 (2001) (unpublished)}\BibitemShut {NoStop}%
...
\bibitem [{gea()}]{geant}%
  \BibitemOpen
  \href@noop {} {}\bibinfo {note} {R.~Brun, F. Carminati, and S. Giani, GEANT~3.21, CERN Program
...
\bibitem [{\citenamefont {Verkerke}\ and\ \citenamefont
  ...
  {eConf No.}\ }\textbf {\bibinfo {volume} {C0303241}} (\bibinfo {year} {2003})},\
...
\bibitem [{\citenamefont {Asner}\ \emph {et~al.}()\citenamefont {Asner}
  ...
  {arXiv:1010.1589v2} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {Asner}\ and\ \citenamefont {Sun}(2006)}]{cpcorr}%
  ...
  \bibinfo {pages} {034024} (\bibinfo {year} {2006})}; {\bibinfo {author} {\bibfnamefont {D.~M.}\ \bibnamefont
  {Asner}}\ and\ \bibinfo {author} {\bibfnamefont {W.~M.}\ \bibnamefont
  {Sun}}, }\bibinfo  {journal} {Phys. Rev. D} {{\bf 
...
\bibitem [{\citenamefont {Bianco}\ \emph {et~al.}(2003)\citenamefont {Bianco},
  ...
  {author} {\bibfnamefont {I.}~\bibnamefont {Bigi}},\ }\href@noop {} {\bibfield
   {journal} {\bibinfo  {journal} {Nuovo Cimento Soc. Ital. Fis.}\ }\textbf {\bibinfo
